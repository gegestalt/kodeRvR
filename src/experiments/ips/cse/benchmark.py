...
from ips.analysis.evidence import evaluate_detailed, network_policy
from ips.research_adoptions import optimal_stopping_policy, train_prioritized_dqn
from ips.adapters.cse_temporal import log_shadow_decision, split_events_by_role
...
def run() -> pd.DataFrame:
    if not EVENTS.exists():
        raise FileNotFoundError(f"run python -m experiments.ips.cse.build_events first: {EVENTS}")
